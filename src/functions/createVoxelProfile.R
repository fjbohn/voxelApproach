...
CreateVoxelProfile <- function(remsen, coord = NA, variable, resolution = 10000, sampling = "quadratic", 
                               dist = 1, sample.size = 100, shape = "square", method = "presence",
                               return.fineMAP = F, voxelsize = 1, side = 10000^0.5, sens = 0.2, 
                               k = 0.3, MCH = T) {
  ...
  for (i in 1:nrow(coord)) {
    ...
    if (method == "presence") {
      voxelDist[[i]] <- findVoxelDistribution(remsen, coord[i, 1:2], side, shape, method = "presence")
    }
    
    if (method == "beamTransformed") {
      voxelDist[[i]] <- findVoxelDistribution(remsen, coord[i, 1:2], side, shape, method = "beamTransformed",sens = sens, k = k)
    }
    
    if (method == "vfp10") {
      voxelDist[[i]] <- findVoxelDistribution(remsen, coord[i, 1:2], side, shape, method = "vfp10")
    }
